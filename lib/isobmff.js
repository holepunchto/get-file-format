...
function getBrandInfo(buffer) {
  const info = {}
  if (buffer.length >= 12) info.majorBrand = toFourCC(buffer, 8, 12)
  ...
  for (let i = 0; i < compatibleCount; i++) {
    const index = 16 + i * 4
    info.compatibleBrands.push(toFourCC(buffer, index, index + 4))
  }
...
async function detectAt(buffer, reader, size, opts = {}) {
  ...
  if (brandInfo.majorBrand) result.majorBrand = brandInfo.majorBrand
