const { lookup, readAt } = require('./util')
...
function countCompatibles(size, length) {
  const end = Math.min(size, length)
  return Math.max(0, Math.floor((end - 16) / 4))
}
...
async function refineAt(format, reader, size, opts) {
  if (!opts.inspectTracks || format !== 'mp4') return format
...
async function detectAt(buffer, reader, size, opts = {}) {
  ...
  const majorBrand = buffer.subarray(8, 12)
  const format = lookup(ftyp, majorBrand)
  if (format) return refineAt(format, reader, size, opts)

  const compatibleCount = countCompatibles(boxSize, buffer.length)
  for (let i = 0; i < compatibleCount; i++) {
    const index = 16 + i * 4
    const compatible = buffer.subarray(index, index + 4)
    const format = lookup(ftyp, compatible)
