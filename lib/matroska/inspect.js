...
const BYTE_BASE = 256
const VINT_ID_MAX_LENGTH = 4
const VINT_SIZE_MAX_LENGTH = 8
const ELEMENT_HEADER_MAX_LENGTH = VINT_ID_MAX_LENGTH + VINT_SIZE_MAX_LENGTH
const UNSIGNED_MAX_LENGTH = 8
...
async function readElementAt(reader, offset, end) {
  ...
  const header = await readAt(reader, offset, Math.min(ELEMENT_HEADER_MAX_LENGTH, end - offset))
  const id = readVint(header, 0, VINT_ID_MAX_LENGTH, false)
  if (!id) return null

  const size = readVint(header, id.length, VINT_SIZE_MAX_LENGTH, true)
  if (!size) return null
...
async function readUnsignedAt(reader, start, end) {
  ...
  if (length < 1 || length > UNSIGNED_MAX_LENGTH) return null
